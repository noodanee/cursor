...
function getDiagnosticsPositions(editorView) {
...
function renderBlocks(scrollbar, diagnosticsPositions) {
...
function createScrollbarElement(diagnosticsPositions) {
...
const CursorScrollbar = ViewPlugin.fromClass(class {
    constructor(view) {
        this.element = null;
        this.margin = { right: 64 };
        this.diagnosticsPositions = [];
        this.diagnosticsPositions = getDiagnosticsPositions(view);
        this.tryToInitElement(view);
    }
    tryToInitElement(view) {
