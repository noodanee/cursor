...
export const syntaxBundle = (filePath) => __awaiter(void 0, void 0, void 0, function* () {
    if (!filePath)
        return [];
    ...
    const langPackage = languages.find((lang) => lang.extensions.includes(fileExtension) ||
        (lang.filename && fileName.match(lang.filename)));
    if (langPackage != null) {
        extensions.push(yield langPackage.load());
    }
    else {
        // Otherwise, we  handle special cases
        const otherExtension = usedExtensions.find((ext) => ext.extensions.includes(fileExtension) ||
            ext.exactMatches.some((match) => fileName.match(match)));
        if (otherExtension != null) {
            const langPackage = languages.find((lang) => lang.name.includes(otherExtension.origLangName));
            if (langPackage != null) {
                extensions.push(yield langPackage.load());
            }
        }
